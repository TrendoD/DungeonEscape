...
/// <summary>
/// Editor script untuk setup GameOverUI dan Fade Overlay di scene GameOver.
/// Akses melalui menu: Tools > Dungeon Escape > Setup GameOver Scene
/// </summary>
public class GameOverSetupEditor : EditorWindow
{
    [MenuItem("Tools/Dungeon Escape/Setup GameOver Scene")]
    public static void ShowWindow()
    {
        GetWindow<GameOverSetupEditor>("GameOver Setup");
    }

    void OnGUI()
    {
        GUILayout.Label("GameOver Scene Setup Tool", EditorStyles.boldLabel);
        EditorGUILayout.Space(10);

        EditorGUILayout.HelpBox(
            "Tool ini akan membuat:\n" +
            "1. GameObject 'GameOverManager' dengan script GameOverUI.cs\n" +
            "2. Canvas 'FadeCanvas' dengan Image hitam full screen (alpha = 1)\n" +
            "3. Auto-assign FadeOverlay ke GameOverUI\n\n" +
            "PENTING: Jalankan ini HANYA di scene GameOver!",
            MessageType.Info);

        EditorGUILayout.Space(10);

        if (GUILayout.Button("Setup GameOver + Fade UI", GUILayout.Height(40)))
        {
            SetupGameOver();
        }
    ...
    void SetupGameOver()
    {
        // 1. Cari atau buat GameOverManager
        ...
        if (existingUI != null)
        {
            ...
            Debug.Log("GameOverSetup: GameOverUI sudah ada, menggunakan yang existing.");
        }
        else
        {
            managerObject = new GameObject("GameOverManager");
            managerObject.AddComponent<GameOverUI>();
            Debug.Log("GameOverSetup: GameOverManager baru dibuat.");
        }

        GameOverUI gameOverUI = managerObject.GetComponent<GameOverUI>();
        ...
        if (canvasObj != null)
        {
            ...
            Debug.Log("GameOverSetup: FadeCanvas sudah ada.");
        }
        else
        {
            // Buat Canvas baru
            ...
            Debug.Log("GameOverSetup: FadeCanvas baru dibuat.");
        }
        ...
        if (existingOverlay != null)
        {
            ...
            Debug.Log("GameOverSetup: FadeOverlay sudah ada.");
        }
        else
        {
            // Buat Image baru
            ...
            Debug.Log("GameOverSetup: FadeOverlay Image dibuat (alpha = 1, hitam penuh).");
        }

        // 4. Assign FadeOverlay ke GameOverUI
        gameOverUI.fadeOverlay = fadeOverlay;

        // 5. Mark scene sebagai dirty agar perubahan tersimpan
        UnityEditor.SceneManagement.EditorSceneManager.MarkSceneDirty(
            UnityEngine.SceneManagement.SceneManager.GetActiveScene());

        // Pilih GameOverManager di Hierarchy
        Selection.activeGameObject = managerObject;

        EditorUtility.DisplayDialog(
            "Setup Complete", 
            "GameOverManager dan FadeCanvas berhasil dibuat!\n\n" +
            "Jangan lupa:\n" +
            "1. Save scene (Ctrl+S)\n" +
            "2. FadeOverlay sudah diset hitam penuh (alpha = 1)\n" +
            "3. Saat scene dimulai, akan fade out untuk menampilkan Game Over",
            "OK");

        Debug.Log("GameOverSetup: Setup selesai!");
    }
    ...
    void ApplyDarkAtmosphere()
    {
        ...
        if (mainCamera != null)
        {
            ...
            Debug.Log("GameOverSetup: Camera background diset ke hitam.");
        }
        else
        {
            Debug.LogWarning("GameOverSetup: Main Camera tidak ditemukan!");
        }
        ...
        Debug.Log("GameOverSetup: Dark atmosphere applied!");
    }
    ...
    void CreateVignetteEdges(GameObject parentCanvas)
    {
        ...
        Debug.Log("GameOverSetup: Vignette edges dibuat.");
    }
